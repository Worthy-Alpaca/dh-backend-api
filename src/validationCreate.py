...
for m in mList:
    df = pd.read_csv(
        os.getcwd()
        + os.path.normpath(f"/data/all/validation/{m['machine']}DataFrame_raw.csv"),
        low_memory=False,
    )
    ...
    m10Data = pd.DataFrame(m10Data, columns=["Class", "Type", "Time", "Placements"])
